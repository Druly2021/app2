...
class PlayerLevel(models.Model):
    ...
    def assign_price_for_level(self, prize):
        if self.is_completed:
            LevelPrize.objects.create(Level=self.level, prize=prize, received=timezone.now())
    ...
    def export_player_levels_to_csv():
        ...
        for player_level in player_levels:
            ...
            ). prize_title if LevelPrize.objects.filter(level=player_level.level).exists() else ''
            writer.writerow([player_level.player.player_id, player_level.level.title,
                              player_level.is_completed, prize_title])
